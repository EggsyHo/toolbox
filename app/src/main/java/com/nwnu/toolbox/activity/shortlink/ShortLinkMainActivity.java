package com.nwnu.toolbox.activity.shortlink;
...
import com.nwnu.toolbox.util.ClipboardUtil;
...
public class ShortLinkMainActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void generateLink() {
        String key = "61c4764baee1e859d95636ed@8584ce2c9ea979374bfca212b5eda25b";
        String input = inputTxt.getText().toString().trim();
        if (TextUtils.isEmpty(input)) {
            Snackbar.make(inputTxt, "请先输入网址", Snackbar.LENGTH_LONG).show();
        } else {
            String url = "http://api.suowo.cn/api.htm?url=urlencode('" + input + "')&key=" + key + "&expireDate=2030-03-31";
